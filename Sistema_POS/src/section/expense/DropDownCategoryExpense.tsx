...
import { deleteCategory } from '../../api/purchase/expense';

export default function DropDownCategoryExpense({ isOpen, selectedCategory, onLoadCategory, onRefreshCategory }: { isOpen: boolean; selectedCategory: CategoryExpenseProps[]; onLoadCategory: () => void; onRefreshCategory: (selectedCategory: CategoryExpenseProps[]) => void }) {
  ...
  const allActive = selectedCategory.every(category => category.state);
  const allInactive = selectedCategory.every(category => !category.state);
  ...
  const handleStateChangeClick = async () => {
    ...
    if (allActive) {
      ...
      for (const category of selectedCategory) {
        const { success, error } = await deleteCategory(category.IDExpenseCategory, { state: !category.state });
        if (!success) {
    ...
    if (newState !== null) {
      for (const category of selectedCategory) {
        const { success, error } = await deleteCategory(category.IDExpenseCategory, { state: newState });
        if (!success) {
          showToast(error.message, false);
        }
      }
  ...
  return (
    <>
      ...
      {isShowModal && <CategoryExpenseModal isSave={false} onClose={onClose} selectedCategory={selectedCategory[0]} />}
    </>
  )
}
